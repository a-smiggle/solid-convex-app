...
export function DashboardOverviewPage() {
  // Simulate loading state for demo; replace with real data fetching logic
  const [loading] = createResource(async () => {
    ...
    <section class="motion-enter-fade-up space-y-6">
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Show when={!loading()} fallback={
          <>
            <SkeletonCard title="MRR" lines={2} />
            <SkeletonCard title="Active Users" lines={2} />
            <SkeletonCard title="Churn" lines={2} />
          </>
        }>
      ...
      <Show when={!loading()} fallback={<SkeletonChart title="Revenue Pipeline" bars={5} />}>
